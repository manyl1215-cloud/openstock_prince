...
def get_today_sheet():
    """包含自動重試與強制初始化的 Google Sheets 連線邏輯"""
    creds_dict = json.loads(GCP_KEY)
    creds = Credentials.from_service_account_info(creds_dict, scopes=['https://www.googleapis.com/auth/spreadsheets'])
    
    for attempt in range(5): # 增加到 5 次重試
        try:
...
def run_morning_report():
    ...
    # 一次性寫入 D2:F，大幅減少 API 負擔
    sheet.update('D2:F' + str(len(morning_data) + 1), morning_data)
    
    msg = f"🌅 *今日早盤正溢價名單*\n----------------------------\n"
    msg += "\n".join(positive_gaps) if positive_gaps else "今日開盤低迷，無正溢價標的。"
    return msg
...
def run_after_hours_report():
    ...
    try:
        ...
        twse = pd.DataFrame(requests.get(twse_url, headers=headers, timeout=15).json()['data'])
        tpex = pd.DataFrame(requests.get(tpex_url, headers=headers, timeout=15).json()['aaData'])
    except:
        return "⚠️ 盤後資料抓取失敗，伺服器繁忙。"
    ...
    for sym, name in stock_dict.items():
        ...
        try:
            ...
            if abs(f+t) >= 200:
                msg_list.append(f"`{sid}` {name}: `{f+t:+d}張`")
        except:
...
if __name__ == "__main__":
    try:
        ...
        if now.weekday() < 5:
            ...
            send_telegram_msg("💡 今日週末，股市休市中。")
    except Exception as e:
        send_telegram_msg(f"🆘 系統執行異常: {str(e)}")
